...
本插件的机密信息（数据库密码、API 密钥等）支持**两种配置方式**，二者可同时存在，也可只配置其中一种：

- **插件配置（WebUI）**：在 AstrBot 的 WebUI 插件界面中图形化填写 `New API 网站配置` 与 `数据库配置`。
- **`.env` 文件**：在插件根目录创建 `.env` 文件。

> **优先级规则**：如果某个字段在插件配置和 `.env` 中**同时存在**，以**插件配置**为准；如果只存在于其中一方，则使用存在的那一方；两方都为空则初始化失败。

### **方式一：通过 `.env` 文件**

请在您的 **插件根目录** (与 `main.py` 同级) 创建一个名为 `.env` 的文件，并填入以下内容：
...
在 AstrBot 的 **WebUI 插件界面** 中，打开本插件的配置面板，填写 `New API 网站配置` 和 `数据库配置` 分组下的对应字段即可。

剩下的所有功能性配置，例如签到奖励额度、打劫成功率、消息模板、监控群号等，都可以在同一个 WebUI 插件界面中进行图形化配置，非常方便。
