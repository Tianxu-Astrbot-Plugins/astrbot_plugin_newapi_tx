...
- **配置灵活**：插件配置（WebUI）与 `.env` 双源混合，自动建表无需手动操作。
...
- **插件配置（WebUI）**：在 AstrBot 插件界面中填写 `New API 网站配置` 与 `数据库配置`。
- **`.env` 文件**：在插件根目录创建 `.env`。
...
在 AstrBot 的 WebUI 插件界面中打开本插件配置面板，填写 `New API 网站配置` 与 `数据库配置` 分组下的字段即可。其余功能配置（签到奖励、打劫概率、消息模板、监控群号等）均可图形化配置。
...
| 指令 | 说明 |
| --- | --- |
| `/查余额 [网站ID或QQ号]` | 查询指定用户网站余额（智能识别 ID） |
| `/解绑 [网站ID]` | 强制解除指定网站 ID 的绑定关系 |
| `/查询 [网站ID或QQ号]` | 智能查询绑定关系（ID ↔ QQ 互查） |
| `/调整余额 [ID] [额度]` | 调整用户额度，正数增加、负数减少，如 `/调整余额 12345 100` |
